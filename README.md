...
As of version v1.1.7 (13 April 2022), the software consists of the following:

* 209 Java code files organized into 16 packages
* 5,045 blank lines inserted for code readability
* 12,534 comment lines written for code understandability (take a look and tell me if I succeeded)
* 13,589 Java code lines
...
1. As of release 1.1.7, I have fixed the problem of the software possibly taking too long to run if [-x ncnt](#-x-ncnt) is set too large. The software now caps how many subsets of investments it checks. It should never hang no matter how large <b>ncnt</b> is. The reason? The software now currently ignores the value of <b>ncnt</b>. The trade-off now, however, is that the software may not find the best rebalance solution for an account in these circumstances. The standard-deviation may not be optimally low, and it is even possible that the software may not be able to zero the residual if an account contains too many investments in the same category. If the user does not have too many investments in the same category (e.g., twenty small-cap value stock ETFs), then this will not be a problem. The to-do for this item is to change the meaning of the [-x ncnt](#-x-ncnt) preference to be the cap on number of checked investment subsets, and reintegrate use of the value. Currently, the cap is hard-coded at 1,000. Do not forget to change the documentation in this markdown file once that is done! <i>Target: release 1.2+.</i> 

...
As of version v1.1.6 (4 April 2022), the software consists of the following:

* 208 Java code files organized into 16 packages
* 4,976 blank lines inserted for code readability
* 12,327 comment lines written for code understandability (take a look and tell me if I succeeded)
* 13,429 Java code lines
...
1. In the sections marked [-x ncnt](#-x-ncnt) and [How Does Rebalancing Work](#how-does-rebalancing-work), I have talked about what the software does if it cannot precisely balance an account. What I mean by "precisely" is this: to the penny (or whatever the subdivision of currency under consideration) - given the assigned weights. I call the amount that cannot be rebalanced precisely <b>the residual</b>, and this residual can be positive, or negative. The software will examine all subsets of its children - be they weight subtypes, or individual tickers - and attempt to place the residual such that: 1) the residual is minimized, ideally zero, and; 2) the standard deviation from ideal is minimized. Currently, what I do is create a collection of bitmaps, with bit set if the corresponding child is to receive residual, or bit clear if the child is not to receive residual. The size of the collection is 2**n, where "n" is the number of children. I then sort the collection by number of bits set. <b>But this is dumb because it can be so memory intensive for large "n".</b> The enhancement is to create an algorithm that programmatically calculates then next integer in the sequence, and with thought this should not be hard to do. As well, I recommend that the subset search algorithm be capped so that it does not perform too many more iterations of subset searching once residual has been reduced to zero. This may result in a standard deviation that is not optimal, but would you rather have software that hangs because <b>ncnt</b> is too large? <i>Target: release 1.7+.</i>

...
As of version v1.4.4 (26 July 2022), the software consists of the following:
...
1. It occurred to me in August 2022 that there is a bond fund category that is lacking: Municipal bonds. In order to add this category, there will need to be a code added to the [Code File](#code-file). The number of ordinary ASCII alphabetic characters are already used up, so I suggest a distinguished, special character, or a character outside the traditional ASCII set (Þ, or thorn, anybody?). A new municipal bond type will need to be inserted in the <i>FundType</i> enumeration, and this type then associated with the chosen character code. The <i>Ticker</i> class will need a new entry in its association map (search for any of the other bond subtypes, and follow the example). The <i>TickersBuilder</i> class will need an update to introduce municipal bonds in a way that is consistent with the other bond subtypes. The [DetailedFile](#detailed-file) will need a new weight type at <b>Level 2</b> weight category under bond investments. Finally, any entries in the [Ticker File](#ticker-file) that are associated with municipal bonds will need to declare the character associated with the type. This list of tasks to accomplish the addition of municipal bonds may not be exhaustive.    
